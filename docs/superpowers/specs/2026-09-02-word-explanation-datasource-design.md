...
설계 시점의 미결 사항은 실제 빌드를 돌려 전부 해소됐다. 아래는 측정값과 그에 따른 결정이다.

### 14.1 산출물 크기

첫 빌드는 mwe-index 5.32MB(315,263개) + inflections 11.22MB로 **첫 실행에 17.3MB**를 받게 했다.
설계가 정한 경고선의 17배였다. 세 가지를 바꿔 **2.02MB**로 줄였다.

- **mwe-index를 품사로 걸렀다** — `verb`, `phrase`, `prep_phrase`, `proverb`, `adv`, `intj`만 남긴다.
  전체의 73%가 명사 복합어(`banjo timepiece`, `health clubs`)와 지명(`arzier-le muids`)이라 자막 학습에
  무가치했다. 벌크 덤프에는 최상위 `categories` 필드가 없어(kaikki.org의 단어별 페이지에만 있다)
  "English phrasal verbs" 범주로 거를 수 없었고, 품사가 유일하게 쓸 수 있는 신호였다.
  5.32MB → 1.29MB (72,069개).
- **inflections를 한정했다** — `ko`가 비어 있지 않은 표제어와, 남은 다어절 표제어의 첫 토큰만 넣는다.
  굴절형 되돌리기가 값을 내는 건 한국어 뜻이 있는 흔한 단어와 `gave up` 매칭 두 경우뿐이다.
  11.22MB → 0.72MB (37,529개, 482,609개에서 축소).
- **버킷 키를 세 글자로 넓혔다** — 두 글자일 때 `co.json`이 4.51MB였다. 클릭 한 번에 4.5MB를 받는다는 뜻이다.
  세 글자로는 버킷 11,409개, 최대 `_.json` 1.86MB, **중앙값 0.5KB**, 상위 5%가 80KB다.

사전 본체는 1.35M 표제어를 그대로 유지한다. §2.2가 한국어 뜻이 없는 구동사의 폴백으로 영영 정의를
쓰기로 했고, 세 글자 버킷이면 조회당 비용이 이미 중앙값 0.5KB다. Hosting 저장 165MB는 무료 한도 10GB 안이다.

### 14.2 한국어 커버리지의 실제 수치

전체 1,350,821개 표제어 중 `ko`를 가진 것은 **15,612개(1.16%)** 다. 이 수치는 en.wiktionary가
고유명사·전문용어·굴절형까지 담고 있어서 희석된 값이며, **자막에 실제로 나오는 흔한 단어의 커버리지는
§2.2의 실측(일반 단어 30/30, 구동사 31/60)이 여전히 맞다.** 실기기 확인에서 `College`, `first`, `of`는
한국어 뜻이 나왔고 `drop out`은 영영 정의로 폴백했다.

### 14.3 알려진 한계 — 고치지 않고 남긴 것

- **ko.wiktionary 전용 표제어는 버려진다.** 버킷은 kaikki 항목을 기준으로 만들고 `koMap`을 조회해
  붙이는 방식이라, kaikki에 없고 ko.wiktionary에만 있는 표제어는 사전에 실리지 않는다. §2.2가 말한
  합집합이 아니라 좌측 조인이다. kaikki가 영어 표제어 1.35M을 담고 있어 실제 누락은 극소수지만,
  고치려면 빌드 수정과 11,409개 파일 재배포가 필요해 미뤘다.
- **분리 허용 간격 3토큰이 모든 다어절 유형에 적용된다.** §8 규칙 2는 동사와 불변화사 사이를 뜻했는데,
  구현은 관용구·속담에도 같은 규칙을 쓴다. 제대로 고치려면 빌드가 분리 가능 표현을 표시해 줘야 한다.
  누적 간격 계산이 피해 범위를 제한한다.
- **하이픈·숫자가 든 다어절 표제어는 매칭되지 않는다.** 토큰 정규화가 `[^a-z']`를 제거하므로 그런
  표제어는 인덱스의 죽은 무게다. 동작상 버그는 아니다.
- **약 382개(0.03%)의 `ko`에 확장되지 않은 `{{...}}` 템플릿 문법이 남아 있다.**

### 14.4 사전 갱신

수동 재빌드다. `npm run build:dict` 후 `firebase deploy --only hosting`. 위키 덤프는 매달 갱신되지만
이 사전이 그 속도를 따라갈 이유는 없다.
